...
**Feature hooks — workouts (`features/workouts/hooks/`):** check before implementing any workout-related logic.
  ...
  - `useWorkoutSession(session, body_weight_kg)` → `{ exercises, active_index, is_loading, is_finishing, rest_timer_active, rest_timer_duration, total_rest_seconds, goToExercise, updateDraftSet, confirmSet, addSet, removeSet, finishWorkout, dismissRestTimer }`. Core active-session state. Pre-fills `weight_kg` for `reps_only` exercises using `body_weight_kg × (effective_bw_factor ?? 0.6)`. `updateDraftSet` accepts `'weight_kg' | 'reps' | 'duration_seconds' | 'distance_km'`.
